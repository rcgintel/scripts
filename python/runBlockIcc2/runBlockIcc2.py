...
parser.add_option("-D", "--doit", dest="doit", default="nil", help="is this invoked by doit might not have to remove the directory")
...
for block in blocks:
    ...
    env = env.strip()
    ex_file.write(env)
    if (("PTLA0" in project) or ("WCLA0" in project) or ("NVLA0" in project)):
        data = "\n\nfc -file ./icc2Run.tcl"
    else:
    ...
    data = "\n\nopen_lib -read "+ndmLocation+"/"+block+".ndm; open_block -read "+block+"\n\n"
    ex_file2.write("\nset blockName "+block)
    ex_file2.write(data)
    if option.tcl != "nil":
