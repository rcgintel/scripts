...
def show_report(location):
    if globalVariable.displayResult == 1:
...
def set_app_var(option):
    ...
    float_pattern = r'^[+-]?\d+.\d+$'
    if re.match(integer_pattern, appValue):
        appValue = int(appValue)
    elif re.match(float_pattern, appValue):
        appValue = float(appValue)
    elif re.match(bool_pattern, appValue):
        appValue = bool(appValue)
    else:
        appValue = str(appValue)
        ...
        for path in returnData:
            ...
            for cor, ww in childData:
                ...
                try:
                    ...
                    #code.interact(local=locals())
                childCommand = "-nosplit -input_pins -capacitance -nets -transition  -from "+path[1]+" -to "+path[2]+" -through {"+path[3].replace(":"," ")+"}"
                #childCommand = childCommand + " -nosplit -input_pins -capacitance -nets -transition "
                #breakpoint()
                #code.interact(local=locals())
